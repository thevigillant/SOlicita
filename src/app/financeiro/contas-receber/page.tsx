...
import { CheckCircle, BarChart, Coins, Pencil, Trash2, Check } from "lucide-react";
...
const empty = { valor: "", dataVencimento: "", pago: false, dataPagamento: "", ordemId: "" };
...
export default function ContasReceberPage() {
    ...
    const totalRecebido = items.filter(i => i.pago).reduce((s, i) => s + i.valor, 0);

    const openCreate = () => { setEditing(null); setForm(empty); setError(""); setShowModal(true); };
    const openEdit = (item: Conta) => {
        setEditing(item);
        setForm({ valor: String(item.valor), dataVencimento: item.dataVencimento.split("T")[0], pago: item.pago, dataPagamento: item.dataPagamento?.split("T")[0] || "", ordemId: item.ordemId || "" });
    ...
    const handleSave = async () => {
        ...
        const url = editing ? `/api/financeiro/contas-receber/${editing.id}` : "/api/financeiro/contas-receber";
    ...
    const handleDelete = async (id: string) => {
        ...
        await fetch(`/api/financeiro/contas-receber/${id}`, { method: "DELETE" });
        load(page);
    };

    const markPaid = async (item: Conta) => {
        await fetch(`/api/financeiro/contas-receber/${item.id}`, {
            method: "PUT",
    ...
    return (
        <AdminLayout title="Contas a Receber" subtitle={`${total} registros`}>
            <div className="row g-3 mb-4">
                <div className="col-md-4">
                    <div className="stat-card">
                        <div className="d-flex justify-content-between align-items-start">
                            <div><div className="stat-label">Pendente</div><div className="stat-value" style={{ fontSize: "18px", marginTop: 6 }}>{fmt(totalPendente)}</div></div>
                ...
                <div className="col-md-4">
                    <div className="stat-card">
                        <div className="d-flex justify-content-between align-items-start">
                            <div><div className="stat-label">Recebido</div><div className="stat-value" style={{ fontSize: "18px", marginTop: 6 }}>{fmt(totalRecebido)}</div></div>
            ...
            <div className="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
                ...
                <button id="btn-nova-conta-receber" className="btn btn-primary" onClick={openCreate}>+ Nova Conta</button>
            </div>
                    ...
                    <table className="table table-hover">
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr><td colSpan={6}><div className="empty-state"><div className="empty-state-icon"><Coins size={48} className="text-secondary" /></div><p>Nenhuma conta encontrada</p></div></td></tr>
                            ) : items.map((item) => (
                                <tr key={item.id} className={isVencida(item.dataVencimento, item.pago) ? "table-warning" : ""}>
                                    <td>{item.ordem?.numeroOF || "—"}</td>
                                    <td><strong className="text-success">{fmt(item.valor)}</strong></td>
                                    ...
                                    <td className="text-end">
                                        {!item.pago && <button className="btn btn-sm btn-outline-success me-1 d-flex align-items-center d-inline-flex gap-1" onClick={() => markPaid(item)}><Check size={14} /> Recebido</button>}
                    ...
                    <div className="modal-dialog">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h5 className="modal-title">{editing ? "Editar Conta" : "Nova Conta a Receber"}</h5>
